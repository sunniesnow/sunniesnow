...
Sunniesnow.I18n = {
	...
	async init() {
		...
		for (const [lang, langName] of Object.entries(I18nData.languages)) {
	...
	async sheet(name) {
		...
		const data = I18nData.sheets[`${name}-${this.lang}`];
		if (!data) {
			throw new TypeError(`No such i18n sheet: ${name}-${this.lang}`);
		}
		const result = (this.sheets[this.lang] ??= {})[name] = Object.assign({}, data);
		if (this.lang !== this.FALLBACK_LANG) {
			const fallbackSheet = (this.sheets[this.FALLBACK_LANG] ??= {})[name] ??= Object.assign({}, I18nData.sheets[`${name}-${this.FALLBACK_LANG}`]);
