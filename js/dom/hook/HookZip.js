...
Sunniesnow.HookZip = class HookZip extends Sunniesnow.Hook {
	initialize() {
		...
		this.setting.addEventListener('loadstart', event => this.zipEntries.forEach(s => s.onZipStartLoading(event.token)));
	}

	async apply(value, token) {
		const result = await JSZip.loadAsync(value);
		this.zipEntries.forEach(s => s.onZipLoaded(result, token));
